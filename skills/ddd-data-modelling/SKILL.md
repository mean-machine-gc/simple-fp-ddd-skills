...
description: >
  ...
  per primitive. Produces a clean types.ts. Use when the user wants to model a domain,
  design types, or start a new feature from the data model up.
...
- If **yes — lifecycle states** (with valid transitions):
  ...
  the specific state they require as input.

  ```ts
  type EmptyCart     = { status: 'empty';     id: string }
  type ActiveCart    = { status: 'active';    id: string; customerId: string; items: CartItem[] }
  type ConfirmedCart = { status: 'confirmed'; id: string; customerId: string; items: CartItem[]; confirmedAt: Date }
  type Cart          = EmptyCart | ActiveCart | ConfirmedCart
...
type Parse<T>  = (raw: unknown) => Result<T>
...
type CustomerId = string
type CustomerIdFailure = 'not_a_string' | 'not_a_uuid'
...
type ConfirmedOrder   = { ... }
type Order            = UnconfirmedOrder | ConfirmedOrder
```

Then say:

> "Here is your `types.ts`. The next step is the **examples skill** — it will turn
> your failure unions into a validated behavioural contract, before we write a
...
- If the user seems in a hurry, slow down:
  ...
  > now than after we've built the examples and tests."
