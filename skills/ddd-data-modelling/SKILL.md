...
**Note:** `Result<T, F, S>` is NOT declared in `types.ts` — it lives in
`shared/spec-framework.ts` and is imported from there. Domain types files
should not duplicate infrastructure types.

Then say:

> "Here is your `types.ts`. If this is a new project, run **ddd-init** first to
> set up the shared infrastructure (`shared/spec-framework.ts`, CLI scripts).
>
> Then use **ddd-spec** to capture the behavioral contract for each function —
...
- **Result lives in spec-framework.ts, not types.ts.** Do not redeclare it.
